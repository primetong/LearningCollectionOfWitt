...
public class ImageDownloader {
	 private static final String TAG = "ImageDownloader";  
	    ...
	    {  
	        ...
	        String imageName = "";  
	        if(url != null){  
	            imageName = SDHelper.getInstance().getImageName(url);  
	        }  
	        ...
	        //文件中也没有，此时根据mImageView的tag，即url去判断该url对应的task是否已经在执行，如果在执行，本次操作不创建新的线程，否则创建新的线程。  
	            ...
	            if(mImageView != null){  
	                Log.i(TAG, "执行MyAsyncTask --> " + SDHelper.flag);  
	                SDHelper.flag ++;  
	                task.execute();  
	                //将对应的url对应的任务存起来  
	                map.put(url, task);  
	    ...
	    private boolean needCreateNewTask(ImageView mImageView){  
	        ...
	        if(mImageView != null){  
	            ...
	            if(imageCaches.get(curr_task_url)==null)return true;
	    ...
	    private Bitmap getBitmapFromFile(Activity mActivity,String imageName,String path){  
	        ...
	        if(imageName != null){  
	            ...
	            String real_path = "";  
	            try {  
	                if(SDHelper.getInstance().hasSDCard()){  
	                    real_path = SDHelper.getInstance().getExtPath() + (path != null && path.startsWith("/") ? path : "/" + path);  
	                }else{  
	                    real_path = SDHelper.getInstance().getPackagePath(mActivity) + (path != null && path.startsWith("/") ? path : "/" + path);  
	                }  
	    ...
	    private class MyAsyncTask extends AsyncTask<String, Void, Bitmap>{  
	        ...
	        protected Bitmap doInBackground(String... params) {  
	            ...
	            if(url != null){  
	                try {  
	                    ...
	                    String imageName = SDHelper.getInstance().getImageName(url);  
	        ...
	        protected void onPostExecute(Bitmap result) {  
	            //回调设置图片
	        	Bitmap b=Bitmap.createScaledBitmap(result, 100, 100, true);
	            if(download != null){  
	                download.onDownloadSucc(b,url,mImageView);  
